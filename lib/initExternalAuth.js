      ...
      try {
        ziti.ziti_init_external_auth( controllerHost, async ( cfg ) => {
            ...
            function onAuthEvent(event) {
                // console.log(`onAuthEvent: event: `, event);
    
                if (event.action === 'login_external') {
                    // console.log(`Received login_external event, type: ${event.type}, detail: ${event.detail}`);
                } else if (event.action === 'select_external') {
                    // console.log(`Received select_external event, type: ${event.type}, detail: ${event.detail}`);

                    // console.log(`calling ziti.extAuthToken`);
                    let ret = ziti.extAuthToken(token)
                    // console.log(`ziti.extAuthToken returned: ${ret}`);

                } else if (event.action === 'cannot_continue') {
                    // console.log(`Received cannot_continue event, type: ${event.type}, detail: ${event.detail}`);
                } else if (event.action === 'prompt_totp') {
                    // console.log(`Received prompt_totp event, type: ${event.type}, detail: ${event.detail}`);
                } else if (event.action === 'prompt_pin') {
                    // console.log(`Received prompt_pin event, type: ${event.type}, detail: ${event.detail}`);
                } else {
                    // console.log(`Received unknown auth event: ${JSON.stringify(event)}`);
                }
